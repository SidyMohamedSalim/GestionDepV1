                ...
                <tr
                    ...
                    <td class="flex items-center justify-center gap-4 px-6 py-4">
                        ...
                        <div class="space-y-6 text-red-500 ">
                            <button x-data=""
                                x-on:click.prevent="$dispatch('open-modal', 'confirm-vacataire-deletion{{ $enseignant->id }}')">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                    fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                            ...
                            <x-modal name="confirm-vacataire-deletion{{ $enseignant->id }}" focusable>
                                <form method="post" action="{{ route('vacataire.destroy', $enseignant) }}" class="p-6">
                                    @csrf
                                    @method('delete')

                                    <h2 class="text-lg font-medium text-gray-900">
                                        {{ __('Voulez vous vraiment Supprimers ?') }}
                                    </h2>
